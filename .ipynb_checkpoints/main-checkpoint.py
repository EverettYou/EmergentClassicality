...
class Dipolar1D(dist.Distribution):
    ...
    def log_prob(self, x):
        # probability is given by p(x) = (1 + x * m)/2
        # but we renormalize p(x) by its neutral value 1/2
        # this can prevent baseline entropy to accumulate in the loss
        p = 1 + x.squeeze(dim = -1) * self.m
        return torch.log(p)
...
class Dipolar3D(dist.Distribution):
    ...
    def log_prob(self, x):
        ...
        # probability is given by p(x) = (1 + x3 * m)/(4*pi)
        # but we renormalize p(x) by its neutral value 1/(4*pi)
        # this can prevent baseline entropy to accumulate in the loss
        p = 1 + x3 * self.m
        return torch.log(p)
...
class VAE(nn.Module):
    ...
    def loss(self, x):
        ...
        # kl divergence loss
        kl_loss = torch.sum((q.loc**2 + q.scale**2 - 1.)/2 - torch.log(q.scale))
        # total loss
    ...
    def learn(self, steps=1, batch_size=1):
        ...
        for i in range(steps):
            ...
            self.optimizer.step()
        self.step += steps
        total_size = batch_size * self.apparatus.sample_size * steps
        train_loss = run_loss/total_size
        return train_loss
    ...
    def test(self, batch_size=1):
        ...
        with torch.no_grad():
            ...
            loss = self.loss(xs)
            run_loss += loss.item()
        total_size = batch_size * self.apparatus.sample_size
        test_loss = run_loss/total_size
        return test_loss